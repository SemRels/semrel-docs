...
<Aside type="tip">
  Both regular image variants include `git` and `ca-certificates` — semrel works out of the box in either.
...
GitHub mounts the action workspace with the runner's ownership and requires
Docker container actions to run as root. Use the dedicated `-action` tag only
with `uses: docker://...`; the regular images deliberately remain non-root.
...
- The opt-in `-action` image runs as root solely for GitHub Docker container
  actions. Do not use it for general container workloads.
