...
3. Run **condition-phase** plugins — abort if any fail
4. Check that the current branch is configured for release
5. Find the last git tag and collect commits since then
6. Parse commits against Conventional Commits rules
7. Calculate the next SemVer version (major / minor / patch)
8. Generate changelog and release notes
9. Run **pre-tag** plugins (e.g. version file updaters)
10. Commit all tracked release changes once
11. Create and push the git tag
12. Run **release-phase** plugins (packagers, publishers, providers, hooks)
...
| `--commit-msg-file <path>` | Read a single commit message from a file (mutually exclusive with `--stdin`) |
...
# Lint from a file — the standard shape for a git commit-msg hook
# (.git/hooks/commit-msg calls hooks with the message file path as $1)
...
`semrel release` performs the CLI and configured-plugin update check
automatically. It is advisory; network failures do not block a release.
...
Orchestrate releases across all packages in a monorepo workspace with a single command.
Configure the workspace in the root `.semrel.yaml` — see the [Monorepo guide](/guide/monorepo/) for full details.
...
Release all workspace packages in dependency order.
