...
plugins:
  ...
  - uses: @semrel/hook-slack
    # yaml-language-server: $schema=https://registry.semrel.io/schemas/plugins/hook-slack/latest.json
    args:
...
| Plugin (registry name) | `https://registry.semrel.io/schemas/plugins/{name}/v1.json` |
| Plugin latest | `https://registry.semrel.io/schemas/plugins/{name}/latest.json` |

Use the canonical registry name, including its category prefix (for example,
`provider-github` or `hook-slack`).
